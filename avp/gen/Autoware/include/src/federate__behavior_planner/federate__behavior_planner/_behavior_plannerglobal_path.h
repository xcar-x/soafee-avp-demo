...
#ifndef __BEHAVIOR_PLANNERGLOBAL_PATH_H // necessary for arduino-cli, which automatically includes headers that are not used
...
#endif
typedef struct _behavior_plannerglobal_path_self_t{
    self_base_t base; // This field is only to be used by the runtime, not the user.
    int end[0]; // placeholder; MSVC does not compile empty structs
} _behavior_plannerglobal_path_self_t;
typedef struct {
    token_type_t type;
    lf_token_t* token;
    size_t length;
    ...
    #endif
} _behavior_plannerglobal_path_port0_t;
typedef struct {
    token_type_t type;
    lf_token_t* token;
    size_t length;
    ...
    interval_t physical_time_of_arrival;
    #endif
} _behavior_plannerglobal_path_port_t;
#endif
#endif
